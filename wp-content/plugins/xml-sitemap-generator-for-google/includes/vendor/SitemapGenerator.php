...
class SitemapGenerator {
	 ...
	 */
	public function addUrls( $urlsArray, $google_news = false ) {
		if ( ! is_array( $urlsArray ) ) {
			throw new \InvalidArgumentException( "Array as argument should be given." );
		}

		$callback = $google_news ? 'addNewsUrl' : 'addUrl';

		foreach ( $urlsArray as $url ) {
			$this->{$callback}($url[0] ?? null, $url[1] ?? null, $url[2] ?? null, $url[3] ?? null, $url[4] ?? null );
		}
	}
	 ...
	 * @param string $publication_name Publication Name
	 * @param string $language Publication Language
	 ...
	 */
	public function addNewsUrl( $url, $publication_name, $language,  $title, $lastModified = null ) {
		if ( null === $url ) {
		...
		$tmp                     = array();
		$tmp['loc']              = $url;
		$tmp['publication_name'] = $publication_name;
	 ...
	 */
	public function createSitemap( $is_google_news = false ) {
		if ( ! isset( $this->urls ) ) {
		...
		$xsl_template  = $is_google_news ? 'sitemap-news' : 'sitemap' ;
		$xsl_url       = plugins_url( "templates/{$xsl_template}.xsl", GRIM_SG_BASENAME ) . '?ver=' . GRIM_SG_VERSION;
		$generatorInfo = '<!-- generator="GoogleXmlSitemapGenerator" -->
								<!-- sitemap-generator-url="https://wpgrim.net" sitemap-generator-version="1.0" -->';

		$newsHeader         = $is_google_news ? ' xmlns:news="http://www.google.com/schemas/sitemap-news/0.9"' : '';
		$sitemapHeader      = '<?xml version="1.0" encoding="UTF-8" ?>
								<?xml-stylesheet type="text/xsl" href="' . $xsl_url . '"?>' . $generatorInfo . '
								<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"' . $newsHeader . '></urlset>';
			...
			foreach ( $sitemap as $url ) {
				...
				if ( $is_google_news ) {
					...
					if ( isset( $url['publication_name'] ) ) {
						...
						$publication->addChild( 'xmlns:news:name', $url['publication_name'] );
					...
					if ( isset( $url['title'] ) ) {
						$news->addChild( 'xmlns:news:title', $url['title'] );
					}

//					$news->addChild( 'xmlns:news:keywords', implode( ', ', apply_filters( 'sgg_news_keywords', array() ) ) );
//					$news->addChild( 'xmlns:news:stock_tickers', implode( ', ', apply_filters( 'sgg_news_stock_tickers', array() ) ) );
				} else {
					if ( isset( $url['lastmod'] ) ) {
